...
export default function AddLead() {
    ...
    const [jobTitles, setJobTitles] = React.useState<components["schemas"]["JobTitle"][]>([]);

    const [budgetRanges, SetBudgetRanges] = React.useState<components["schemas"]["RangeMoney"][]>([]);
    const [deliveryRanges, setDeliveryRanges] = React.useState<components["schemas"]["RangeInt"][]>([]);
    const [propertyTypes, setPropertyTypes] = React.useState<components["schemas"]["PropertyType"][]>([]);
    const [areas, setAreas] = React.useState<components["schemas"]["Area"][]>([]);

    const [employees, setEmployees] = React.useState<components["schemas"]["Employee"][]>([]);
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (data.get('budgetRange') !== '') {
            body['interests'][0]['budget_range_id'] = parseInt(data.get('budgetRange'));
        }

        if (data.get('deliveryRange') !== '') {
            body['interests'][0]['delivery_range_id'] = parseInt(data.get('deliveryRange'));
        }

        if (data.get('propertyType') !== '') {
            body['interests'][0]['property_type_id'] = parseInt(data.get('propertyType'));
        }
        ...
        await getData('/accounts/sales', HttpMethod.POST, undefined, body).then((data) => {
